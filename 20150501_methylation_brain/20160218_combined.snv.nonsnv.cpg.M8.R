...
pdf("/lustre/sblab/martin03/repository/20150921_BrainMethylomeRoadMap/snv/figures/20160218_combined.snv.nonsnv.cpg.M8.boxplot.pdf", width= 14/2.54, height= 10/2.54)
ggplot(snv.nonsnv.m, aes(x = factor(type_met, levels(factor(type_met))[c(2,1,3)]), y = pct_met, fill = factor(ID, levels(factor(ID))[c(2,1)]))) + geom_boxplot(outlier.shape = NA, aes(colour = factor(type_met, levels(factor(type_met))[c(2,1,3)]))) + theme_bw() + theme(legend.title = element_blank()) + scale_fill_manual(values=c("#999999", "#E69F00")) + scale_colour_manual(values = c("turquoise3", "green", "red"), labels=c("5mC","5hmC","C/other")) + xlab("") + ylab("% Methylation") + scale_x_discrete(labels = c("5mC","5hmC","C/other"))
dev.off()

pdf("/lustre/sblab/martin03/repository/20150921_BrainMethylomeRoadMap/snv/figures/20160218_combined.snv.nonsnv.cpg.M8.boxplot.nocolour.pdf", width= 14/2.54, height= 10/2.54)
ggplot(snv.nonsnv.m, aes(x = factor(type_met, levels(factor(type_met))[c(2,1,3)]), y = pct_met, fill = factor(ID, levels(factor(ID))[c(2,1)]))) + geom_boxplot(outlier.shape = NA) + theme_bw() + theme(legend.title = element_blank()) + scale_fill_manual(values=c("#999999", "#E69F00")) + xlab("") + ylab("% Methylation") + scale_x_discrete(labels = c("5mC","5hmC","C/other"))
dev.off()

pdf("/lustre/sblab/martin03/repository/20150921_BrainMethylomeRoadMap/snv/figures/20160218_combined.snv.nonsnv.cpg.M8.violin.pdf", width= 14/2.54, height= 10/2.54)
ggplot(snv.nonsnv.m, aes(x = factor(type_met, levels(factor(type_met))[c(2,1,3)]), y = pct_met, fill = factor(ID, levels(factor(ID))[c(2,1)]))) + geom_violin(scale = "area", aes(colour = factor(type_met, levels(factor(type_met))[c(2,1,3)])), draw_quantiles = 0.5) + theme_bw() + theme(legend.title = element_blank()) + scale_fill_manual(values=c("#999999", "#E69F00")) + scale_colour_manual(values = c("turquoise3", "green", "red"), labels=c("5mC","5hmC","C/other")) + xlab("") + ylab("% Methylation") + scale_x_discrete(labels = c("5mC","5hmC","C/other"))
dev.off()
